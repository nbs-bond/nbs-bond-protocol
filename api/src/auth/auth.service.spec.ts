...
describe('AuthService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: {
            isValidPublicKey: jest.fn().mockImplementation((addr: string) => addr === validAddress),
          },
        },
      ],
    }).compile();
  ...
  describe('generateChallenge', () => {
    it('throws BadRequestException for invalid Stellar address', async () => {
      await expect(service.generateChallenge('INVALID_ADDRESS')).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('refreshToken', () => {
    it('returns new access token for valid token', async () => {
      jwtService.verify.mockReturnValue({ sub: validAddress, kycStatus: 'VERIFIED' } as any);

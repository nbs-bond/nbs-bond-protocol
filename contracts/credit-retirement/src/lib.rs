...
impl CreditRetirement {
    ...
    pub fn retire_credits(
        ...
        // Validate by-type balance before proceeding
        // For Basket credits, we must check the specific type balance, not just combined
        // For BlueCarbon, the storage type is Carbon (internally)
        let storage_type = if credit_type == CreditType::BlueCarbon {
            CreditType::Carbon
        } else {
            credit_type
        };
        let accrued_by_type: i128 = env.invoke_contract(
            ...
            vec![
                ...
                storage_type.into_val(&env),
            ],
        );
        if amount > accrued_by_type {
            return Err(CreditError::InsufficientCreditsByType);
        }
        ...
        // Also keep the combined check as a safety net
        if amount > accrued {
            return Err(CreditError::InsufficientCredits);
        }
...
mod test {
    ...
    fn test_retire_more_than_accrued_rejected() {
        ...
        assert_eq!(result, Err(Ok(CreditError::InsufficientCreditsByType)));
    }
